              ...
              <Col xs={12} className="mt-3 text-center text-md">
                <span className="font-weight-bold">
                  {this.props.names[this.state.result[0].lane.toLowerCase()]
                    ? `${
                        this.props.names[
                          this.state.result[0].lane.toLowerCase()
                        ]
                      } `
                    : ' '}
                </span>
                ...
                .map((obj, index) => {
                  return (
                    <tr key={`podium-${index}`}>
                      <th scope="row">
                        <img
                          src={`./material/svg/${index + 1}.svg`}
                          height="27"
                          width="27"
                          alt={`${index}logo`}
                        />
                      </th>
                      <td>{this.props.names[obj.lane.toLowerCase()]}</td>
                      <td>
                        {index > 0
                          ? this.state.result[0].numberOfLaps -
                              obj.numberOfLaps >=
                            1
                            ? `+ ${this.state.result[0].numberOfLaps -
                                obj.numberOfLaps} L`
                            : this.calculateGap(
                                this.state.result[0].ends,
                                obj.ends
                              )
                          : '-'}
                      </td>
                      <td>{obj.best / 1000}</td>
                      <td>{obj.numberOfLaps}</td>
                      <td>{obj.lane}</td>
                    </tr>
                  )
                })}
